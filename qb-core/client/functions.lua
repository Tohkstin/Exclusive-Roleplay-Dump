...
function QBCore.Functions.HasItem(item)
    -- TODO: Swap out contents with single line qb-inventory export
...
function QBCore.Functions.Notify(text, texttype, length, icon)
    ...
    if type(text) == "table" then
        ...
        message.text = text.message or text.text or "Placeholder"
        message.caption = text.caption or "Placeholder"
    end

    local executed = pcall(function()
        exports["mc9-notify"]:QBNotify(message)
    end)
...
function QBCore.Functions.Announce(text, length)
...
function QBCore.Functions.ShowStatusNotification(title, text)
    SendNUIMessage({
        action = 'showStatusNotification',
        title = title,
        text = text
    })
end

function QBCore.Functions.HideStatusNotification()
    SendNUIMessage({
        action = 'hideStatusNotification'
    })
end
...
function QBCore.Functions.AwaitCallback(name, ...)
    local p = promise.new()
...
function QBCore.Functions.Progressbar(name, label, duration, useWhileDead, canCancel, disableControls, animation, prop, propTwo, onFinish, onCancel)
    ...
    exports["qb-inventory"]:ProgressBarLock(true)
    exports['progressbar']:Progress({
        ...
        exports["qb-inventory"]:ProgressBarLock(false)
...
function QBCore.Functions.GetClosestVehicles(coords, distance)
    local ped = PlayerPedId()
    local vehicles = GetGamePool('CVehicle')
    ...
    for i = 1, #vehicles, 1 do
        if #(GetEntityCoords(vehicles[i]) - coords) <= distance then
            table.insert(closestVehicles, vehicles[i])
        end
    end
...
function QBCore.Functions.SetVehicleProperties(vehicle, props)
	if DoesEntityExist(vehicle) then
        ...
        pcall(function()
            exports["mc9-tuning"]:applyTune(vehicle)
        end)
	end
end
...
RegisterNUICallback('sendNotification', function(data, cb)
    QBCore.Functions.Notify(data.text, data.type, data.length)
    cb({})
end)
